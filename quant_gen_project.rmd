...
fstat_mle_list_test <- pmap(list(xa = data.frame(x_a), xd = data.frame(x_d), MLE = mle_gene1), fstat_calc, pheno[, 1])
fstat_mle_list_smpl <- flatten_dbl(fstat_mle_list_test)
pval_fstat_mle_list <- pf(fstat_mle_list_smpl, 2, 344 - 3, lower.tail = FALSE)

hist(pval_fstat_mle_list, breaks = 20, 
     xlab = "p-values", 
     ylab = "Number of p-values", 
     main = "Distribution of p-values")

man_plot_data_col1 <- as.data.frame(cbind(1:ncol(geno), pval_fstat_mle_list))
...
ggplot(man_plot_data_col1, aes(x = x, y = -log(pval, base = 10))) +
  ...
  ylab("-log10(pVal)")
summary(which(pval_fstat_mle_list < 0.05 / ncol(geno)))
