...
Each SNP position was tested for Hardy-Weinberg Equilibrium using the exact test described in [Wigginton *et al* (2005)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1199378/). In this analysis, the test was used as implemented by the HardyWeinberg R package: [HardyWeinberg Package on CRAN](https://cran.r-project.org/package=HardyWeinberg). The result determined if the proportion of the alleles at a particular genotype position were as expected. A statistically significant result could indicate a problem with the sequencing process or a population structure, both of which can lead to misleading GWAS results.
...
```{r block9_gwas_simpl_lin_reg_no_covar, comment=NA, include=FALSE}
# x_a matrix creation
...
### Manhattan plot ###
(pval_no_covar_man_plot <- pval_no_covar %>% 
  gather("gene", "pval", 2:6) %>% 
...
(pval_no_covar_qqplot_ggplot <- pval_no_covar_qqplot %>% 
...
```{r block9a_gwas_simpl_lin_reg_no_covar_incl_plots, comment=NA, echo=FALSE, fig.height=4, fig.width=8}
plot_grid(pval_no_covar_man_plot, pval_no_covar_qqplot_ggplot, nrow = 1, ncol = 2)
```
...
```{r block10_pheno_covars_sig_tests, include=FALSE, comment=NA}
...
```{r block11_gwas_with_given_pop_covar, comment=NA, include=FALSE}
### Linear regression with Population as covariate
# create data frame with combined phenotype and covariate data, to make sure the matching is right
...
pval_ERAP2_w_pop_num_covar <- map2(
  # map2 applies the function lin_reg_fstat_w_covar over each column of x_a and x_d 
  data.frame(x_a), data.frame(x_d),  
  # the function below carries out the test of each genotype
  lin_reg_fstat_w_covar, 
  # these two arguments determine the phenotype tested and the covariate matrix used
...
```{r block12_gwas_with_full_geno_PC1, comment=NA, include=FALSE}
...
```{r block15_gwas_with_full_geno_PC1toPC10, comment=NA, include=FALSE}
...
### Manhattan plot ###
(pval_PC1_to_PC10_covar_man_plot <- pval_PC1_to_PC10_covar %>% 
  gather("gene", "pval", 2:6) %>% 
  ggplot(aes(x = x, y = -log(pval, base = 10), color = gene)) +
  geom_point(alpha = 0.8, size = 2) +
  ggtitle("Manhattan Plot\nLinear Regression\nGenotype PC1-PC10 Covariates\nAll Genes") +
  xlab("Genotype Position") +
  ylab("-log10(pValue)") +
  ylim(0, 100) +
  geom_hline(yintercept = -log(0.05 / ncol(geno_HW_filt), base = 10), size = 1, alpha = 0.8) + 
...
(pval_PC1_to_PC10_covar_qqplot_plot <- pval_PC1_to_PC10_covar_qqplot %>% 
  gather("gene", "neglog10_pval", 2:6) %>% 
  ggplot(aes(x = expected, y = neglog10_pval, color = gene)) +
  geom_point(alpha = 0.8, size = 2) +
  geom_abline(intercept = 0, slope = 1, col = "black", alpha = 0.5, size = 2) +
...
```{r block16_gaston_linear_mixed_model, comment=NA, include=FALSE}
# need a different package
library(gaston)
# this package requires a different data type for the getnotype data
geno_bed_matrix <- as(as.matrix(geno_HW_filt), "bed.matrix")
# create a matrix of covriance between individuals in data 
...
pval_lmm_PC1_to_PC3_covar_qqplot %>% 
  ...
  ggtitle("QQ Plot\nLinear Mixed Model, PC1-PC3 as Covariates\nAll Genes") +
  ylim(0, 100)
### Individual genes alone, to show that the QQ Plots are still not ideal
### FAHD1 ###
pval_lmm_PC1_to_PC3_covar_qqplot %>% 
  ggplot(aes(x = expected, y = FAHD1)) +
  geom_point(alpha = 0.8, size = 2) +
...
```{r block17_concluding, echo=FALSE, fig.height=3, fig.width=8}
plot_grid(pval_PC1_to_PC10_covar_man_plot, pval_PC1_to_PC10_covar_qqplot_plot, nrow = 1, ncol = 2)
```

```{r block18_summary_of_PC1_to_PC10_hits, include=FALSE}
### ERAP2 hits SNP names and locations ###
PC1_to_PC10_ERAP2_hits <- as.data.frame(
  row.names(pval_PC1_to_PC10_covar)[which(pval_PC1_to_PC10_covar$ERAP2 < 0.05 / ncol(geno_HW_filt))],
  stringsAsFactors = FALSE
  )
colnames(PC1_to_PC10_ERAP2_hits) <- "id"
# join to SNP info dataset in order to get chromosome and position info
PC1_to_PC10_ERAP2_hits <- PC1_to_PC10_ERAP2_hits %>%
  left_join(snp_info, by = "id") %>% 
  arrange(chromosome, position) %>% 
  # is the SNP within the gene itself? Numbers from gene info file
  mutate(
...
# what percent of significant SNPs are within the gene coding region?
sum(PC1_to_PC10_ERAP2_hits$in_gene) / nrow(PC1_to_PC10_ERAP2_hits) * 100
# which chromosomes are the SNP hits located on?
unique(PC1_to_PC10_ERAP2_hits$chromosome)
# what is the range of positions? 
summary(PC1_to_PC10_ERAP2_hits$position)

### FAHD1 hits SNP names and locations #
PC1_to_PC10_FAHD1_hits <- as.data.frame(
  row.names(pval_PC1_to_PC10_covar)[which(pval_PC1_to_PC10_covar$FAHD1 < 0.05 / ncol(geno_HW_filt))],
  stringsAsFactors = FALSE
  )
colnames(PC1_to_PC10_FAHD1_hits) <- "id"
# join to SNP info dataset in order to get chromosome and position info
PC1_to_PC10_FAHD1_hits<- PC1_to_PC10_FAHD1_hits %>%
  left_join(snp_info, by = "id") %>% 
  arrange(chromosome, position) %>% 
  # is the SNP within the gene itself? Numbers from gene info file
  mutate(
...
# what percent of significant SNPs are within the gene coding region?
sum(PC1_to_PC10_FAHD1_hits$in_gene) / nrow(PC1_to_PC10_FAHD1_hits) * 100
# which chromosomes are the SNP hits located on?
unique(PC1_to_PC10_FAHD1_hits$chromosome)
# what is the range of positions? 
summary(PC1_to_PC10_FAHD1_hits$position)

### PEX6 hits SNP names and locations ###
PC1_to_PC10_PEX6_hits <- as.data.frame(
  row.names(pval_PC1_to_PC10_covar)[which(pval_PC1_to_PC10_covar$PEX6 < 0.05 / ncol(geno_HW_filt))],
  stringsAsFactors = FALSE
  )
colnames(PC1_to_PC10_PEX6_hits) <- "id"
# join to SNP info dataset in order to get chromosome and position info
PC1_to_PC10_PEX6_hits <- PC1_to_PC10_PEX6_hits %>%
  left_join(snp_info, by = "id") %>% 
  arrange(chromosome, position) %>% 
  # is the SNP within the gene itself? Numbers from gene info file
  mutate(
...
# what percent of significant SNPs are within the gene coding region?
sum(PC1_to_PC10_PEX6_hits$in_gene) / nrow(PC1_to_PC10_PEX6_hits) * 100
# which chromosomes are the SNP hits located on?
unique(PC1_to_PC10_PEX6_hits$chromosome)
# what is the range of positions? 
summary(PC1_to_PC10_PEX6_hits$position)
```

In conclusion, 41 SNPs were found to be related to ERAP2 expression, 44% of which were in the coding region of the gene, and all were on the same chromosome in nearby regions. All of the 6 FAHD1 hits were within the gene coding region. Lasly, 15 SNPs were found to have a statistically significant effect on PEX6 expression. The last section of this code can be uncommented to export the results to a csv for further analysis.

```{r results, include=FALSE}
######## Uncomment and run to get all of the results out #######
# write.csv(PC1_to_PC10_ERAP2_hits, "ERAP2_SNP_hits")
# write.csv(PC1_to_PC10_PEX6_hits, "PEX6_SNP_hits")
# # write.csv(PC1_to_PC10_FAHD1_hits, "FAHD1_SNP_hits")
```
